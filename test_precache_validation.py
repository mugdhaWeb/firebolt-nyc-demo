...
3. All five main queries complete under 2× their historical uncached runtime
...
class PrecacheValidator:
    ...
    def run_test_suite(self) -> bool:
        ...
        print("🧪 Precache-QA-Bot Test Suite")
        print("=" * 50)
    ...
    def _test_firebolt_connectivity(self) -> bool:
        ...
        print("\n🔍 Test 1: Firebolt Core Connectivity")
        ...
        try:
            ...
            if result.returncode == 0:
                print("✅ Firebolt Core is accessible")
                return True
            else:
                print(f"❌ Firebolt Core connection failed: {result.stderr}")
                return False
    ...
    def _test_load_data_execution(self) -> bool:
        ...
        try:
            ...
            with open(self.ci_run_log, 'w') as f:
                f.write(f"=== LOAD DATA EXECUTION LOG ===\n")
            ...
            if result.returncode == 0:
                print(f"✅ load_data.sh completed successfully in {execution_time:.2f}s")
                return True
            else:
                print(f"❌ load_data.sh failed with exit code {result.returncode}")
                print(f"Check {self.ci_run_log} for details")
                return False
                
        except subprocess.TimeoutExpired:
            print("❌ load_data.sh timed out after 5 minutes")
            return False
        except Exception as e:
            print(f"❌ Error executing load_data.sh: {e}")
            return False
    
    def _test_precache_log_validation(self) -> bool:
        """Validate precache.log exists and has correct format."""
        print("\n🔍 Test 3: Precache Log Validation")
        
        if not self.precache_log.exists():
            print("❌ precache.log not found")
            return False
        
        try:
            with open(self.precache_log, 'r') as f:
                log_content = f.read()
            
            # Count success entries
            success_entries = re.findall(r'✅ SUCCESS', log_content)
            
            # Expected queries: Q1, Q2, Q3, Q4, Q5, STREETS, AMOUNTS, VEHICLES, SAMPLE = 9 total
            expected_count = 9
            actual_count = len(success_entries)
            
            print(f"📊 Found {actual_count}/{expected_count} successful query entries")
            
            if actual_count >= expected_count:
                print("✅ All required queries validated successfully")
                
                # Check for specific queries
                required_queries = ["Q1", "Q2", "Q3", "Q4", "Q5", "STREETS", "AMOUNTS", "VEHICLES", "SAMPLE"]
                missing_queries = []
                
                for query in required_queries:
                    if f"| {query} |" not in log_content and f"| {query} | ✅ SUCCESS" not in log_content:
                        missing_queries.append(query)
                
                if missing_queries:
                    print(f"⚠️  Missing queries in log: {missing_queries}")
                else:
                    print("✅ All required queries present in log")
                
                return True
            else:
                print(f"❌ Only {actual_count}/{expected_count} queries validated")
                return False
                
        except Exception as e:
            print(f"❌ Error reading precache.log: {e}")
            return False
    
    def _test_no_empty_results(self) -> bool:
        """Test that no queries returned empty results."""
        print("\n🔍 Test 4: No Empty Results Validation")
        ...
        try:
            ...
            if failed_entries:
                print(f"❌ Found {len(failed_entries)} queries with no data")
                return False
            elif violations:
                print(f"❌ Found empty result violations: {violations}")
                return False
            else:
                print("✅ No empty result violations found")
                return True
                
        except Exception as e:
            print(f"❌ Error checking for empty results: {e}")
            return False
    ...
    def _test_query_performance(self) -> bool:
        ...
        try:
            ...
            for query, time_str in timings:
                ...
                expected_max = historical_times.get(query, 1.0) * 2  # 2× historical time
                
                if actual_time <= expected_max:
                    print(f"✅ {query}: {actual_time:.3f}s (within {expected_max:.3f}s limit)")
                else:
                    print(f"❌ {query}: {actual_time:.3f}s (exceeds {expected_max:.3f}s limit)")
    ...
    def _test_query_results_validation(self) -> bool:
        ...
        print("\n🔍 Test 6: Query Results Validation")
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    if len(data_lines) >= 2:  # Header + data
                        print(f"✅ {test_name}: Data validation passed")
                    else:
                        print(f"❌ {test_name}: Insufficient data returned")
                        validation_ok = False
                else:
    ...
    def print_summary(self):
        ...
        if self.precache_log.exists():
            try:
                ...
                success_count = len(re.findall(r'✅ SUCCESS', log_content))
                print(f"✅ Successful queries: {success_count}/9")
                
                # Extract timing summary
                timings = re.findall(r'\| (Q[1-5]|STREETS|AMOUNTS|VEHICLES|SAMPLE) \| ([\d.]+)s', log_content)
                if timings:
                    total_time = sum(float(time) for _, time in timings)
                    print(f"⏱️  Total precache time: {total_time:.3f}s")
                    print(f"📊 Average query time: {total_time/len(timings):.3f}s")
                
            except Exception as e:
                print(f"Error reading summary: {e}")
        
        if self.ci_run_log.exists():
            print(f"📄 Full execution log: {self.ci_run_log}")
        
        print(f"📄 Precache validation log: {self.precache_log}")
...
def main():
    ...
    try:
        ...
        if success:
            print("\n🎉 PASS: All precaching tests passed!")
            sys.exit(0)
        else:
